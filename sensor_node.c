...
#define TEMP_DEV 		5	// max afwijking vorige temperatuur in 0.1 celsius
                            // max deviation to the previous temperature is 0.1 degree celsius.
...
int main( int argc, char *argv[] )
{
  ...
  if (argc != 5) //user input not valid!
  {
    print_help();
    exit(EXIT_SUCCESS);
  }
  else
  {
    ...
    data.id = atoi(argv[1]); // transfer from string to int
    sleep_time = atoi(argv[2]);
    strncpy(server_ip, argv[3],strlen(server_ip)); // copy 'n' bytes of a string to the destination
    server_port = atoi(argv[4]);
  }
  
  srand48( time(NULL) ); //generate a random time
  ...
  while(i) 
  {
    data.value = data.value + TEMP_DEV * ((drand48() - 0.5)/10); //assign a new value to the data.value
    time(&data.ts); //Return the current time and put it in data.ts if it is not NULL.
